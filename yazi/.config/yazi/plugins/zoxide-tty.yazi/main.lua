-- yazi 同梱の zoxide プラグインの差し替え。
--
-- 同梱版は `zoxide query -i` を stderr = PIPED で spawn する。zoxide は内部で
-- fzf を起動し、Debian の fzf 0.44.1 は TUI を stderr に描画するため、`Z` を
-- 押すと画面が真っ暗になり操作不能になる。stderr を端末に継承させて解決する。
-- 継承した分 stderr を取り込めないので、エラー通知は終了コードのみにした。
--
-- fzf を 0.5x 以降(TUI を /dev/tty に描画)へ更新したらこのファイルは不要。
...
function M.run_with(cwd)
	local child, err = Command("zoxide")
		...
		:stderr(Command.INHERIT) -- 同梱版は PIPED。ここが唯一の差分
		:spawn()
	...
	elseif not output.status.success and output.status.code ~= 130 then
		return nil, Err("`zoxide` exited with code %s", output.status.code)
	end
