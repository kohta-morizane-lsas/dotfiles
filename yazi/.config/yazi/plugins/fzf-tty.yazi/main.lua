-- yazi 同梱の fzf プラグインの差し替え。
--
-- 同梱版は fzf を spawn するとき stderr を指定しておらず、yazi の Command は
-- 未指定の stderr を /dev/null に落とす。Debian の fzf 0.44.1 は TUI を
-- stderr に描画するため、`z` を押すと画面が真っ暗になり fzf が操作不能になる。
-- ここでは stderr を端末に継承させるだけで、他は同梱版と同じ。
--
...
function M.run_with(cwd, selected)
	local child, err = Command("fzf")
		...
		:stderr(Command.INHERIT) -- 同梱版との唯一の差分
		:spawn()
